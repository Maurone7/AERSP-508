...
- solve_falkner_skan_eqaution: Solves the Falkner-Skan equation using the solve_ivp function.
...
# Define the Falkner-Skan differential equation
...
# Solve the Falkner-Skan equation for each beta value
for beta in beta_list:
    guess_1, guess_2 = 1, 0.1
    while abs(guess_2 - guess_1) > 1E-9:
...
# Generate and save table in LaTeX format
...
for i, beta in enumerate(beta_list):
    ...
    while abs(guess_2 - guess_1) > 1E-8:
        f_0, f_1 = solve_falkner_skan_eqaution(beta, guess_1)[1][-1], solve_falkner_skan_eqaution(beta, guess_2)[1][-1]
        guess_1, guess_2 = guess_2, secant(guess_1, guess_2, f_1, f_0)
    f, f_prime, f_double_prime = solve_ivp(falkner_skan_differential_equation, t_span=(0, 100), y0=[0, 0, guess_2], args=(beta,), t_eval=eta, method='RK45').y
    plt.plot(f_prime[:5001], eta[:5001], c=cmap(i))
...
falkner_skan_profile = lambda capital_lambda: 2*(2+capital_lambda/6)*(37/315 - capital_lambda/945 - (capital_lambda**2)/9072) - 2*(2+H(capital_lambda)) * lambda_s * beta
